...
def minimax(state, player, possible_transitions, is_terminal, utility, switch_player,
            alpha_beta = False, depth_limit = None, estimate_utility = None, depth = 0,
    ...
    for trans, nextstate in transitions.iteritems():
        ...
        if is_terminal(nextstate):
            u = utility(nextstate, initial_player)
        ...
        if depth == 0: print "utility for", trans, "is", u
    ...
    if depth == 0:
        print "Checked %d states." % checked
        print "best_trans", best_trans, "best_u", best_u
        return best_trans
    else:
        return best_u
